...
// swap 的滑点宏
// 如果实际拿到的数量，小于我事先能接受的最小数量，就直接失败回滚交易
// if actual < min → revert
// if actual < min → revert 是 “交易者风险边界的数学表达”
...
impl ConstantProduct {
    ...
    // price = x * precision / y
    // P = x / y 代码里乘了 precision 只是为了避免浮点数
    ...
    // 按 LP 份额取出资产
    // ratio = (l - a) / l
    // withdraw_x = x - x * ratio
    // withdraw_y = y - y * ratio
